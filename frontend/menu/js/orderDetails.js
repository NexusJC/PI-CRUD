...
document.addEventListener('click', (e) => {
  ...
  // Verificar si ya hay un producto en la lista
  let existing = Array.from(orderList.children).find(li => li.dataset.name === name);
  if (!existing) {
    const li = document.createElement("li");
    li.className = "order-item";
  ...
  updateTotals();
  openOrderDetails(); // Abrir el sidebar automáticamente al agregar un producto
});
...
function updateTotals() {
  ...
  // Habilitar botones cuando haya productos
  if (subtotal > 0) {
...
// Botón para mostrar la orden si hay productos
openSidebarBtn.addEventListener('click', openOrderDetails);

// Delegación de eventos para modificar cantidades o eliminar productos
orderList.addEventListener('click', (e) => {
  const btn = e.target.closest('button');
  if (!btn) return;
